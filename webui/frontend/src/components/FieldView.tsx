...
import type { Point2D, Pose2D, RobotSnapshot } from '../types';
...
export default function FieldView({ snapshot }: FieldViewProps) {
  ...
  const robotPose = status?.pose?.field;
  const robot = mapPoint(robotPose, fieldLength, fieldWidth);
  ...
  const opponents = [...entityArray(statusRecord?.opponents), ...entityArray(perception?.opponents)];
  const obstacles = [...entityArray(statusRecord?.obstacles), ...entityArray(perception?.obstacles)];
  const goalposts = entityArray(perception?.goalposts);
  const markings = entityArray(perception?.markings);
  ...
  const markingMarkers = makeMarkers(markings, 'M', 'marking', fieldLength, fieldWidth);
  ...
  const headingValue = (robotPose as Pose2D | undefined)?.theta;
  const heading = isFiniteNumber(headingValue) ? headingValue : null;
  ...
  return (
    ...
    <div className="field" style={fieldStyle}>
      ...
      {robot && ball && <span className="fieldSegment robotBallLine" style={segmentStyle(robot, ball)} />}
      ...
      {markingMarkers.map(renderMarker)}
      ...
      {robot && (
        <span className="marker robot" style={markerStyle(robot)} title={`self | ${pointText(robot.raw)}`}>
          {heading !== null && <span className="heading" style={{ transform: `rotate(${-heading}rad)` }} />}
          R
        </span>
      )}
    ...
    <div className="fieldLegend">
      ...
      <span><b className="legendDot teammateDot" />teammate</span>
      <span><b className="legendDot opponentDot" />opponent</span>
      <span><b className="legendDot obstacleDot" />obstacle</span>
      <span><b className="legendDot goalpostDot" />goalpost</span>
      <span><b className="legendDot markingDot" />marking</span>
