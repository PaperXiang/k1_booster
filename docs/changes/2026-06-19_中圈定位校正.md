...
## 1. 动机
当前所有几何校正器**只纠平移 (dx,dy)，从不纠朝向 θ**（`2026-06-18_定位策略分析.md` §10.4）——一旦打滑/陀螺漂移导致朝向歪，比赛中无法修复，只能等 SET/入场全局定位。这是团队自评"定位极差"的最大结构性短板。借鉴 B-Human "center circle with center line" 复合特征：用中圈与中线的两个交点**一次解出含朝向的全局位姿**。
...
- 来源：`docs/2026-06-18_定位策略分析.md` §12；P1-1 已修复其依赖的 `SelfLocate2X/_doubleX` 中点笔误。
