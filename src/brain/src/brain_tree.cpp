...
NodeStatus Chase::tick()
{
    ...
    if (
        ...
        && ball.range < brain->config->nearBallRange
    ) {
    ...
    double theta_rb = atan2(ball.posToField.y - brain->data->robotPoseToField.y,
                           ball.posToField.x - brain->data->robotPoseToField.x);
