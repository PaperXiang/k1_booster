...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("ball_prediction.predict_time", 0.25);
    declare_parameter<double>("ball_prediction.min_dt", 0.02);
    declare_parameter<double>("ball_prediction.max_dt", 0.20);
    ...
    declare_parameter<double>("ball_prediction.initial_covariance", 1.0);
...
void Brain::init()
{
    ...
    k1_ball_predictor::Config ballPredictorConfig;
    ballPredictorConfig.enable = config->ballPredictionEnable;
    ballPredictorConfig.predict_time = config->ballPredictionPredictTime;
    ballPredictorConfig.min_dt = config->ballPredictionMinDt;
    ballPredictorConfig.max_dt = config->ballPredictionMaxDt;
    ...
    ballPredictorConfig.initial_covariance = config->ballPredictionInitialCovariance;
...
void Brain::loadConfig()
{
    ...
    get_parameter("ball_prediction.enable", config->ballPredictionEnable);
    get_parameter("ball_prediction.use_for_chase", config->ballPredictionUseForChase);
    get_parameter("ball_prediction.predict_time", config->ballPredictionPredictTime);
    get_parameter("ball_prediction.min_dt", config->ballPredictionMinDt);
    get_parameter("ball_prediction.max_dt", config->ballPredictionMaxDt);
    ...
    get_parameter("ball_prediction.initial_covariance", config->ballPredictionInitialCovariance);
...
void Brain::updateBallPrediction() {
    if (!config->ballPredictionEnable || !ballPredictor) {
        data->ballPredictionValid = false;
        data->ballPredictionPredictedOnly = false;
        data->predictedBallPos.clear();
        return;
    }

    k1_ball_predictor::Observation observation;
    observation.stamp = data->ballDetected ? data->ball.timePoint : get_clock()->now();
    observation.position = k1_ball_predictor::Point2D{data->ball.posToField.x, data->ball.posToField.y};
    ...
    data->ballPredictionPredictedOnly = result.predicted_only;
    data->ballVelocityToField = Point2D{result.velocity.x, result.velocity.y};
    data->ballAccelerationToField = Point2D{result.acceleration.x, result.acceleration.y};
    data->ballPosPredictTime = result.stamp;
    data->predictedBallPos.clear();
    ...
    data->filteredBall = data->ball;
    data->filteredBall.posToField.x = result.filtered_position.x;
    data->filteredBall.posToField.y = result.filtered_position.y;
    data->filteredBall.timePoint = result.stamp;
    updateRelativePos(data->filteredBall);

    data->predictedBall = data->ball;
    data->predictedBall.posToField.x = result.predicted_position.x;
    data->predictedBall.posToField.y = result.predicted_position.y;
    data->predictedBall.timePoint = result.stamp;
    updateRelativePos(data->predictedBall);
}
