...
GroundPlaneConfig LoadGroundPlaneConfig(const YAML::Node &node) {
    GroundPlaneConfig config;
    if (!node["ground_plane"]) {
        return config;
    }

    const auto gp = node["ground_plane"];
    config.enable = as_or<bool>(gp["enable"], config.enable);
    config.debug = as_or<bool>(gp["debug"], config.debug);
    config.update_every_n_frames = as_or<int>(gp["update_every_n_frames"], config.update_every_n_frames);
    config.sample_step = as_or<int>(gp["sample_step"], config.sample_step);
    config.min_depth = as_or<float>(gp["min_depth"], config.min_depth);
    config.max_depth = as_or<float>(gp["max_depth"], config.max_depth);
    config.min_ground_height = as_or<float>(gp["min_ground_height"], config.min_ground_height);
    config.max_ground_height = as_or<float>(gp["max_ground_height"], config.max_ground_height);
    config.ransac_distance_threshold = as_or<float>(gp["ransac_distance_threshold"], config.ransac_distance_threshold);
    config.min_inlier_ratio = as_or<float>(gp["min_inlier_ratio"], config.min_inlier_ratio);
    config.max_normal_tilt_deg = as_or<float>(gp["max_normal_tilt_deg"], config.max_normal_tilt_deg);
    config.force_update_head_pitch_delta_deg = as_or<float>(gp["force_update_head_pitch_delta_deg"], config.force_update_head_pitch_delta_deg);
    config.force_update_head_yaw_delta_deg = as_or<float>(gp["force_update_head_yaw_delta_deg"], config.force_update_head_yaw_delta_deg);
    config.use_cached_plane_when_fit_failed = as_or<bool>(gp["use_cached_plane_when_fit_failed"], config.use_cached_plane_when_fit_failed);
    config.write_to_position = as_or<bool>(gp["write_to_position"], config.write_to_position);
    config.write_to_position_projection = as_or<bool>(gp["write_to_position_projection"], config.write_to_position_projection);
    return config;
}

cv::Point2f GetGroundTargetUv(const DetectionRes &detection) {
    const auto &bbox = detection.bbox;
    if (detection.class_name == "Ball" || detection.class_name == "Person" ||
        detection.class_name == "Opponent" || detection.class_name == "Goalpost") {
...
bool IsValidPoseMeasurement(const Pose &pose) {
    return pose != Pose();
}
...
void VisionNode::Init(const std::string &cfg_template_path, const std::string &cfg_path) {
    ...
    ground_plane_config_ = LoadGroundPlaneConfig(node);
    ground_plane_config_.enable = ground_plane_config_.enable && use_depth_;
    ground_plane_estimator_ = std::make_shared<GroundPlaneEstimator>(ground_plane_config_);
    std::cout << "ground_plane.enable: " << ground_plane_config_.enable << std::endl;
    std::cout << "ground_plane.write_to_position: " << ground_plane_config_.write_to_position << std::endl;
    std::cout << "ground_plane.write_to_position_projection: " << ground_plane_config_.write_to_position_projection << std::endl;
...
void VisionNode::ProcessData(SyncedDataBlock &synced_data, vision_interface::msg::Detections &detection_msg) {
    ...
    GroundPlaneResult ground_plane_result;
    if (ground_plane_estimator_ && ground_plane_config_.enable) {
        ground_plane_result = ground_plane_estimator_->UpdateFromDepth(depth_float, intr_, p_eye2base, timestamp);
        if (ground_plane_config_.debug) {
            std::cout << "ground_plane valid: " << ground_plane_result.valid
                      << ", sampled_points: " << ground_plane_result.sampled_points
                      << ", inlier_ratio: " << ground_plane_result.inlier_ratio
                      << ", reason: " << ground_plane_result.failure_reason << std::endl;
        }
    }
    ...
    for (auto &detection : filtered_detections) {
        ...
        detection_obj.position_confidence = 1;

        if (IsValidPoseMeasurement(pose_obj_by_depth)) {
            detection_obj.position_confidence = 2;
        } else if (ground_plane_config_.enable && ground_plane_estimator_) {
            cv::Point3f ground_position;
            cv::Point2f target_uv = GetGroundTargetUv(detection);
            if (ground_plane_estimator_->IntersectRay(target_uv, intr_, p_eye2base, ground_position)) {
                std::vector<float> ground_vec = {ground_position.x, ground_position.y, ground_position.z};
                if (ground_plane_config_.write_to_position) {
                    detection_obj.position = ground_vec;
                }
                if (ground_plane_config_.write_to_position_projection) {
                    detection_obj.position_projection = ground_vec;
                }
                detection_obj.position_confidence = 2;
                detection_obj.target_uv = {target_uv.x, target_uv.y};
            } else if (ground_plane_config_.write_to_position) {
                detection_obj.position = detection_obj.position_projection;
            }
        } else if (ground_plane_config_.write_to_position) {
            detection_obj.position = detection_obj.position_projection;
        }
    ...
    for (auto &uv : corner_uvs) {
        ...
        if (ground_plane_config_.enable && ground_plane_estimator_) {
            cv::Point3f ground_corner;
            if (ground_plane_estimator_->IntersectRay(uv, intr_, p_eye2base, ground_corner)) {
                corner_pos = ground_corner;
            }
        }
    ...
    for (auto &detection : filtered_detections) {
        if (detection.class_name == "Ball" && detection.confidence > ball_msg.confidence) {
            ...
            if (ground_plane_config_.enable && ground_plane_estimator_) {
                cv::Point3f ground_ball;
                cv::Point2f target_uv = GetGroundTargetUv(detection);
                if (ground_plane_estimator_->IntersectRay(target_uv, intr_, p_eye2base, ground_ball)) {
                    value = {ground_ball.x, ground_ball.y, ground_ball.z};
                }
            }
